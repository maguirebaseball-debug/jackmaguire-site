...
**1. Does this post need custom behavior, embedded maps, interactive elements, data-driven widgets, charts, calculators, cards, or a visual structure that cannot work as a plain text article?**
If yes: 
- For pure-prose essays or research posts, use Markdown in `src/content/blog/` unless there is a concrete reason not to. Plain is the default.
- For true widgets, rankings with generated data, interactive tools, or heavy Astro expressions/components inside the flow: use `.astro` in `src/pages/blog/slug.astro`. Keep the surrounding shell as plain as the function allows. Ask before converting existing custom pages.
...
**3. Is this a straight prose article: structured text, headers, blockquotes, maybe useful links, no custom behavior needed?**
...
**Default when unsure**: start with `.md`. New Markdown posts render in the plain article layout automatically.

**Quick reference**:
| Need | Format |
|------|--------|
| Pure prose, no special layout | `.md` |
| Pure prose essay + citations | `.md` |
...
3. **For cited content, confirm every external link resolves and every claim is grounded** (see Fact-checking and verification). Additionally: grep the source (the .md/.content.md or the .astro) for bare `https?://` strings that are not inside `href=` / `src=` attributes or inside Markdown `[text](...)` links; there must be zero in prose. The `npm run check:indexing` script now fails on bare URLs in standalone .astro files. Bare URLs render as unlinked text and violate the "MUST include direct hyperlinks" rule.
   ...
   **Link verification standard (required, no exceptions):** For every hyperlink in an article, you must either (a) successfully load the page content via WebFetch and confirm it matches the claimed source, or (b) get HTTP 200 from `curl` with a browser user agent (`curl -s -A "Mozilla/5.0..." -o /dev/null -w "%{http_code}" <url>`). A 403 response does NOT count as confirmed, even from well-known domains, because 403s can indicate moved or removed pages and not just bot-blocking. If a link cannot be confirmed by one of these two methods: remove the hyperlink and leave the text unlinked, or find and substitute a working URL for the same source. Never assume a URL works because the organization is real. The organization being real and the URL being valid are separate facts.
...
**Step 8: Directory hub parity check.**
If a new page is being published live under a directory or hub URL pattern such as `/recommendations/...`, `/nyc-intern-guide/...`, or any other index-driven section, update the parent hub page in the same push unless there is an explicit reason not to. Verify both the machine-readable link list and the visible card/list on the parent page so the child page is not orphaned after launch.

## Design philosophy and aesthetics
Individual article pages should be plain by default: Times New Roman, black text, white background, browser-default links, minimal footer, no decorative design. Listing pages, navigation pages, widgets, tools, and existing custom UI can keep their current site styling unless Jack asks to convert them.
