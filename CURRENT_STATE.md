...
Stripe sends paid Snapshot completions to `/api/stripe-ai-audit/` through a narrowly scoped live webhook. The endpoint authenticates the destination token, accepts only completed or asynchronously paid Checkout Sessions, checks live mode, paid status, the $79 amount, USD currency, and matching offer metadata, then sends one server-side Meta `Purchase` event using the Stripe Checkout Session ID as the stable event key. Unpaid, mismatched, duplicated, or unrelated events cannot be promoted into purchases. Meta Graph calls now use v25.0. The privacy policy now covers payments, assessment responses, Stripe, Web3Forms, Vercel, Google Analytics, and Meta business tools.
