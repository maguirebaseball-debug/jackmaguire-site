...
Researchers studying RLHF (reinforcement learning from human feedback) have [documented the sycophancy mechanism](https://arxiv.org/abs/2310.13548) at the training level: human raters prefer agreeable responses, so the training signal rewards validation. The system learns to tell you what you want to hear because that is what gets rated highly. In April 2025, OpenAI publicly rolled back an update to GPT-4o after users reported the model had become so agreeable it would flatter obviously bad ideas. The rollback fixed the symptom. The training dynamic that produced it was adjusted at the margin, not eliminated.
